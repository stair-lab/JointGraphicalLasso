...
#test lambda set
lam_1 <- seq(0.01, 0.5, len = 50)
lam_2 <- c(0.75, 1.0,1.25, 1.5,1.75, 2.0)
...
for(q in 1:replic_num){
    ...
    if(q %% 10 == 0){
        ...
        save(fgl_roc, file=paste("../../results/FGL",replic_num,"_scalefree_long.RData",sep=""))
    }
}
}
...
for(q in 1:replic_num){
    ...
    if(q %% 10 == 0){
        ...
        save(ggl_roc, file=paste("../../results/GGL",replic_num,"_scalefree_long_pleasework.RData",sep=""))
    }
}
}
...
for(q in 1:replic_num){
    ...
    for(i in 1:length(lam_1)){
        ...
        gl_fpr[i] <-avg_fprfnr2(fit00, graph_list[[q]], K)[1]
        gl_fnr[i] <- avg_fprfnr2(fit00, graph_list[[q]], K)[2]
    }
    gl_fpr_list[q] <- list(gl_fpr)
    gl_tpr_list[q] <- list(1-gl_fnr)
    if(q %% 10 == 0){
        gl_roc <- list(list("FPR"=gl_fpr_list,"TPR"=gl_tpr_list))
        save(gl_roc, file=paste("../../results/GL",replic_num,"_scalefree_long_50.RData",sep=""))
    }
}
}
...
if(TRUE){
    ...
    if(q %% 10 == 0){
        ...
        save(ssjggl_roc, file=paste("../../results/ssjggl",replic_num,"_scalefree_long.RData",sep=""))
    }
...
if(TRUE){
    ...
    for(q in 1:replic_num){
        ...
        pb$tick()
        Sys.sleep(1 / ii_size)
        ssjfgl_fpr[i] <-avg_fprfnr(fit00, graph_list[[q]], K)[1]
        ssjfgl_fnr[i] <- avg_fprfnr(fit00, graph_list[[q]], K)[2]
        }
    ...
    if(q %% 10 == 0){
        ...
        save(ssjfgl_roc, file=paste("../../results/ssjfgl",replic_num,"_scalefree_long.RData",sep=""))
    }
