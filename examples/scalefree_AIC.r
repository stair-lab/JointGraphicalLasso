...
tiff(paste(path,ID,"fgl_AIC",".tiff",sep=""), units="in", width=5, height=5, res=600, pointsize = 12)

image(x=lambda.eff,y=lambda.eff,z=aic_vec, col=viridis(256),
      ylab=TeX('$\\lambda_2$'),xlab=TeX('$\\lambda_1$'))
contour(x=lambda.eff,y=lambda.eff,z=aic_vec, add = TRUE,nlevels = 20)
dev.off()
...
tiff(paste(path,ID,"ggl_AIC",".tiff",sep=""), units="in", width=5, height=5, res=600, pointsize = 12)
