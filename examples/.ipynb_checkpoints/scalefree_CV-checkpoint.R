...
print("Computing FGL CV")
if(TRUE){
interval_l = 30
lambda.eff <- seq(0.01, 0.3, len = interval_l)
pb <- progress_bar$new(total = interval_l^2)
cv_vec <- matrix(NA, length(lambda.eff),length(lambda.eff))
for(i in 1:length(lambda.eff)){
    for(j in 1:length(lambda.eff)){
        pb$tick()
        Sys.sleep(1 / (interval_l^2)) 
        cv_vec[i,j] <- CV(data, 5, method="fused", lambda.eff[i], lambda.eff[j])          
    }

}

lambda.eff <- seq(0.01, 0.3, len = interval_l)
png(file=paste(path,ID,"fgl_CV",".png",sep=""),width = 480, height = 480, units = "px", pointsize = 12)
image(x=lambda.eff,y=lambda.eff,z=cv_vec, col=viridis(256),
      ylab=TeX('$\\lambda_2$'),xlab=TeX('$\\lambda_1$'))
contour(x=lambda.eff,y=lambda.eff,z=cv_vec, add = TRUE,nlevels = 20)
dev.off()
    
i_idx <- which(cv_vec == min(cv_vec), arr.ind = TRUE)[1,1]
j_idx <- which(cv_vec == min(cv_vec), arr.ind = TRUE)[1,2]

min(cv_vec)
 which(cv_vec == min(cv_vec), arr.ind = TRUE)
...
for(i in 1:length(lambda.eff)){
    for(j in 1:length(lambda.eff)){
        pb$tick()
        Sys.sleep(1 / (interval_l^2))
        cv_vec[i,j] <- CV(data, 5, method="group", lambda.eff[i], lambda.eff[j])            
    }

}

lambda.eff <- seq(0.01, 0.3, len = interval_l)
png(ile=paste(path,ID,"ggl_CV",".png",sep=""),width = 480, height = 480, units = "px", pointsize = 12)
image(x=lambda.eff,y=lambda.eff,z=cv_vec, col=viridis(256),
      ylab=TeX('$\\lambda_2$'),xlab=TeX('$\\lambda_1$'))
contour(x=lambda.eff,y=lambda.eff,z=cv_vec, add = TRUE,nlevels = 20)
...
pb <- progress_bar$new(total = interval_l^2)
cv_vec <- matrix(NA, length(lambda.eff))
for(i in 1:length(lambda.eff)){
        pb$tick()
        Sys.sleep(1 / (interval_l))
        cv_vec[i] <- CV(data, 5, method="group", lambda.eff[i], 0.)         
}

i_idx <- which(cv_vec == min(cv_vec))
