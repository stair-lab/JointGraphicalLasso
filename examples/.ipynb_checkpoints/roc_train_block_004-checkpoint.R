...
graph_list <- list()
data_list <- list()
print("generate dense 20 data set")
for (k in 1:20){
    set.seed(k)
    graph <- list()
    data <- list()
...
save(graph_list, file="../data/20dataset_block004.RData")
save(data_list, file="../data/20dataset_block004.RData")
}
load("../data/20dataset_block004.RData")
load("../data/20dataset_block004.RData")
...
for(k in 1:20){
    ...
    for(i in 1:length(lambda.eff)){
        for(j in 1:length(lambda.eff)){
            ...
            fgl_fpr[i,j] <-avg_fprfnr(fit00, graph_list[[k]], 5)[1]
            fgl_fnr[i,j] <- avg_fprfnr(fit00, graph_list[[k]], 5)[2]
    ...
    if(k %% 10 == 0){
        ...
        save(fgl_roc, file="../results/FGL20_block004.RData")
    }
}
}
...
for(k in 1:100){
    ...
    for(i in 1:length(lambda.eff)){
        for(j in 1:length(lambda.eff)){
            ...
            ggl_fpr[i,j] <-avg_fprfnr(fit00, graph_list[[k]], 5)[1]
            ggl_fnr[i,j] <- avg_fprfnr(fit00, graph_list[[k]], 5)[2]
        }
    ...
    if(k %% 10 == 0){
        ...
        save(ggl_roc, file="../results/GGL20_block004.RData")
    }
}
if(TRUE){
print("Process GL")
...
for(k in 1:100){
    ...
    for(i in 1:length(lambda.eff)){
            ...
            gl_fpr[i] <-avg_fprfnr(fit00, graph_list[[k]], 5)[1]
            gl_fnr[i] <- avg_fprfnr(fit00, graph_list[[k]], 5)[2]
    ...
    if(k %% 10 == 0){
        ...
        save(gl_roc, file="../results/GL20_block004.RData")
    }
}}
